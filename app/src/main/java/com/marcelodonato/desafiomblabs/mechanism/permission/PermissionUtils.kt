package br.com.versafuel.mechanism.permission
...
class AppPermissionUtils(private val activity: AppCompatActivity,
    ...
    companion object {
        //PERMISSIONS
        val LOCATION_PERMISSION = listOf(ACCESS_FINE_LOCATION, ACCESS_COARSE_LOCATION)
        val PHONE_PERMISSION = listOf(READ_PHONE_STATE)
        val CAMERA_PERMISSION = listOf(CAMERA)
        val APP_PERMISSIONS = listOf(CAMERA, READ_PHONE_STATE)
        val AUDIO_WRITE_CAMERA_READ_PERMISSION =
            listOf(RECORD_AUDIO, WRITE_EXTERNAL_STORAGE, CAMERA, READ_EXTERNAL_STORAGE)
